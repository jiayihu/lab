...
% in the abstract
\begin{abstract}
A microservice architecture is a service-oriented architecture composed of loosely coupled elements that have bounded contexts. Each service has a focused, cohesive set of responsibilities. \cite{microservices-definition}
It comes then with no surprise that a key characteristic of the architecture is that the services are loosely coupled and communicate only via APIs. Each service is required to have its own data store to achieve loose coupling. However, this introduces a new range of obstacles.
...
\bibitem{microservices-definition}
